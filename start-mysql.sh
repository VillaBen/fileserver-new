...
# 创建必要的目录
mkdir -p /tmp/mysql/data /tmp/mysql/run /tmp/mysql/log
chmod 777 /tmp/mysql /tmp/mysql/data /tmp/mysql/run /tmp/mysql/log
...
if kill -0 $MYSQL_PID 2>/dev/null; then
    ...
    # 创建数据库
    mysql --socket=/tmp/mysql.sock -u root -e "CREATE DATABASE IF NOT EXISTS fileserver CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>/dev/null
