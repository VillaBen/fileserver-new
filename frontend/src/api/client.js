// src/api/client.js
import { apiClient } from './index';

export default {
  // 获取系统设置
  async getSettings() {
    return await apiClient.get('/admin/settings');
  },

  // 更新系统设置
  async updateSettings(settings) {
    return await apiClient.put('/admin/settings', settings);
  },

  // 更新 VirusTotal API Key
  async updateVirusTotalApiKey(apiKey) {
    return this.updateSettings({ virustotal_api_key: apiKey });
  },

  // 获取 VirusTotal API Key
  async getVirusTotalApiKey() {
    const response = await this.getSettings();
    return response?.data?.virustotal_api_key || '';
  },

  // 测试 VirusTotal API Key
  async testVirusTotalApiKey(apiKey) {
    return await apiClient.post('/admin/settings/test-virustotal', { apiKey });
  },

  // 测试 SMTP 连接
  async testSmtp(config) {
    return await apiClient.post('/admin/settings/test-smtp', config);
  },

  // 发送测试邮件
  async sendTestEmail(to) {
    return await apiClient.post('/admin/settings/send-test-email', { to });
  }
};
