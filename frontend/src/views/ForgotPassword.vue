...
async function handleSubmit() {
  ...
  try {
    ...
    message.value = i18n.t('resetEmailSent') || 'Password reset link has been sent to your email';
    messageType.value = 'success';
    email.value = '';
    toast.success(message.value);
  } catch (error) {
    const errorMsg = error.response?.data?.error || error.message || 'Failed to send reset email. Please try again.';
