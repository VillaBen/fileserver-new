...
function postRequest(path, body, token, signal) {
...
async function test() {
  ...
  // 4. 测试1：正常上传（对比基准）
  console.log('\n=== 测试1：正常上传小文件 ===');
  const smallData = crypto.randomBytes(10 * 1024); // 10KB
  ...
  try {
    ...
    await postRequest('/files/upload', formData2, token, abortController);
